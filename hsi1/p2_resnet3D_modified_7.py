...
class ResNet_spectra(nn.Module):
    ...
    def __init__(self,
        ...
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        self.fc = nn.Linear(block_inplanes[3] * block_1.expansion, n_classes)
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, stride=1):
        ...
        for i in range(1, blocks):
            layers.append(
                block_2(in_planes=self.in_planes,
                        planes=planes,
...
class MergeNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 n_classes=400):
        super().__init__()
        self.net_1 = ResNet(BasicBlock, [2, 2, 2, 2], get_inplanes(), n_input_channels=n_input_channels, n_classes=n_classes)
        self.net_2 = ResNet_spectra(BasicBlock_1,BasicBlock_2, [1, 1, 1, 1], get_inplanes(), n_input_channels=n_input_channels, n_classes=n_classes)
        self.fc = nn.Linear(get_inplanes()[4], n_classes)
...
def generate_model(model_depth, **kwargs):
    ...
    if model_depth == 10:
        model = ResNet(BasicBlock_1,BasicBlock_2, [1, 1, 1, 1], get_inplanes(), **kwargs)
    elif model_depth == 18:
        model = ResNet(BasicBlock_1,BasicBlock_2, [2, 2, 2, 2], get_inplanes(), **kwargs)
    elif model_depth == 34:
        model = ResNet(BasicBlock_1,BasicBlock_2, [3, 4, 6, 3], get_inplanes(), **kwargs)
    elif model_depth == 50:
