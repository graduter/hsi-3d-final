...
for model_name in model_name_list:
    if model_name in ['resnet3D','resnet2D','squeezenet3D','shufflenet3D','mobilenet3D']:
    ...
    for learning_rate in learning_rate_list:
        for weight_decay in weight_decay_list:
            ...
            if model_name == 'resnet3D':
                net = resnet3D.generate_model(model_depth=18, n_classes=num_output, n_input_channels=1)
            if model_name == 'resnet2D':
                net = models.resnet18(pretrained=False)
                net.conv1 = nn.Conv2d(140, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
                net.fc = nn.Linear(net.fc.in_features, num_output)
            ...
            if model_name == 'squeezenet3D':
                ...
                net.features[0] = nn.Conv3d(1, 64, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1))
            ...
            if model_name == 'mobilenet3D':
                ...
                net.features[0][0] = nn.Conv3d(1, 32, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1), bias=False)
            print(net)
            ...
            with open(results_base_model, 'a') as f:
                f.write('base_model_' + model_name + '_' + str(learning_rate) + ':' + '\n')
                for item in results:
                    f.write('\n')
                    f.write(item)
                    f.write('\n')
